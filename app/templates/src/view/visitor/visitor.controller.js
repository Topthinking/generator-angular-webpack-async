...
function VisitorController($rootScope, $http, $log, toastr, FileUploader) {
  ...
  uploader.onSuccessItem = function(fileItem,response,status,headers){
    if(response.status){
      toastr.success(response.msg || "文件上传成功");
    }else{
      toastr.error(response.msg || "文件上传失败");
    }
  };
  ...
  vm.change_tag = function(tag, row,history) {
    if (tag == 0) {
      //访问路径
  ...
  //获取登记信息
  function leads_info(row,history) {
    if(history){
      $http.post("/api/vistor/history_leads/", {
        "sid": row.sid
      }).then(function(data) {
        data = data.data;
        if (data.status) {
  ...
  function vistor_list() {
    if (vm.source_id != '') {
      ...
      vm.show_vistor_data = false;
      $http.post('/api/guest/get_first_user_info/',{
        "page":vm.currentPage,
  ...
  vm.my_tree_handler = function(branch) {
    ...
    if(vm.source_click==1){
      vm.selection='';
    }else{
      vm.selection=vm.my_data[0].label;
    }
    vm.source_path = [];
    ...
    if (branch.id == "-2") {
      ...
      if (branch.id == "-2" && typeof branch.parent_uid == "undefined") {
        branch.expanded = true;
      } else {
    ...
    angular.forEach(data, function(v) {
      if (typeof v.children != "undefined" && v.children.length) {
        ...
        if (vm.other) {
          ...
          v.show_check = false;
          var _tmp_ = [v.label];
          angular.forEach(vm.source_path,function(vv){
            _tmp_.push(vv);
          });
          vm.source_path = _tmp_;
        }       
      }          
    });
  }
  ...
  function source_data() {
    if (vm.source_id != '') {
      ...
      vm.show_data = false;
      vm.rightActive = '';
      $http.post('/api/source/source_data/', {
        "total_source_id": vm.total_source_id,
        "date_type": vm.date_type,
        "start_date_detail": vm.start_date_detail,
        "end_date_detail": vm.end_date_detail,
  ...
  vm.kpi = function(type) {
    if (!isEmptyObj(vm.line_data)) {
      ...
      vm.source_data.columnar.data.type = "bar";
      vm.source_data.columnar.data.barWidth = "20%";
  ...
  function source_list() {
    $http.post('/api/source/',{
      ...
      data = data.data;
        ...
        if(data.data[0].id!="-2"){
          data.data[0].show_check = false;
        }
        data.data[0].selected = true;
        data.data[0].expanded = true;          
        vm.my_tree_handler(data.data[0]);
        vm.source_click=1;
      })
      .catch(function(err) {
        $log.error(err);
  ...
  /*导出当前页面数据*/
  vm.download_name = "生成报表";
  vm.export_data = function() {
    vm.download_name = "报告生成中...";
    $http.post('/api/guest/export_info/', {
      "start_date_detail": vm.start_date_detail,
      "end_date_detail": vm.end_date_detail,
      "date_type": vm.date_type
    }).then(function(data) {
