...
function SourceController($rootScope, $http, $log, toastr, FileUploader) {
  ...
  vm.my_tree_handler = function(branch) {
    ...
    if(vm.first_branch==0 || sign==1){
      source_data();
    }else{
      if(vm.first_branch==1){
        vm.first_branch = 0;
      }
    }
  };
  ...
  vm.export_data = function() {
    ...
    $http.post('/api/export/', {
      ...
      if (data.status) {
        vm.download_name = "生成报表";
        location.href = '/api/export/excel';
      }
    }).catch(function(err) {
      $log.error(err);
    });
